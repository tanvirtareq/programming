...
int main()
{
	...
	while(t--)
	{
		...
		// cout<<p<<endl;
		int ans=0;
		for(int i=p+1;i<sa.size();i++)
		{
			cout<<i<<" "<<sa[i]<<" "<<lcp[i-1]<<endl;
			if(sa[i]>a.size()+1) break;
			if(lcp[i-1]>=b.size()) ans+=max(0, (int)(a.size()-sa[i]-lcp[i-1]));
			else break;
			// cout<<ans<<endl; 
			// cout<<sa[i]<<endl;
		}
		...
		for(int i=p-1;i>=0;i--)
		{
			if(sa[i]>a.size()) break;
			if(lcp[i-1]>=b.size()) ans+=max(0, (int)(a.size()-sa[i+1]-lcp[i-1]));
			else break;
		}
		// cout<<ans<<nl;
		ans++;
